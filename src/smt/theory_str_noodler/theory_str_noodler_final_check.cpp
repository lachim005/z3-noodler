...
namespace smt::noodler {
    ...
    bool theory_str_noodler::solve_lang_eqs_diseqs() {
        for(const auto& item : this->m_lang_eq_or_diseq_todo_rel) {
            ...
            std::shared_ptr<mata::nfa::Nfa> nfa1 = regex::conv_to_nfa(to_app(left_side), m_util_s, m, alph, false );
            std::shared_ptr<mata::nfa::Nfa> nfa2 = regex::conv_to_nfa(to_app(right_side), m_util_s, m ,alph, false );
