    ...
    protected:
        ...
        std::optional<SolvingState> pop_from_worklist() {
            ...
            while(1) {
                ...
                // Ran out of noodles
                if (!noodle_opt.has_value()) continue;

                util::check_limit(m);
                STRACE(str, tout << "Processing noodle" << (is_trace_enabled(TraceTag::str_nfa) ? " with automata:" : "") << std::endl;);
                SolvingState new_element = SolvingState(element_to_process);
                new_element.noodlification_state = std::nullopt;
                new_element.left_side_vars = std::nullopt;
                new_element.left_side_division = std::nullopt;
                new_element.right_side_division = std::nullopt;
