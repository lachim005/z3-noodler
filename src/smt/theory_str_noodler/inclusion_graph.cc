...
FormulaGraph smt::noodler::FormulaGraph::create_inclusion_graph(const Formula& formula, const std::unordered_set<BasicTerm>& length_vars, const AutAssignment& aut_ass) {
    FormulaGraph splitting_graph{ create_simplified_splitting_graph(formula) };
    return create_inclusion_graph(splitting_graph, length_vars, aut_ass);
}
...
FormulaGraph smt::noodler::FormulaGraph::create_inclusion_graph(FormulaGraph& simplified_splitting_graph, const std::unordered_set<BasicTerm>& length_vars, const AutAssignment& aut_ass) {
    ...
    while (true) {
        ...
        for (const FormulaGraphNode& node: simplified_splitting_graph.get_nodes()) {
            if (erased_nodes.contains(node) || !simplified_splitting_graph.inverse_edges[node].empty()) {
                continue;
            }
            // if node is initial (has no transitions going into it)
            ...
            for (auto x : node.get_real_right_side())
            {
                right_states += aut_ass.at(x)->num_of_states();
            }
            for (auto x : node.get_real_left_side())
            {
